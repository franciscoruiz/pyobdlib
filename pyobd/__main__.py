...
class MyApp(wx.App):
    ...
    class sensorProducer(threading.Thread):
        ...
        def run(self):
            wx.PostEvent(
                self._notify_window, StatusEvent([0, 1, "Connecting...."]))
            self.initCommunication()
            if self.port.state == 0:  # cant connect, exit thread
                self.stop()
                # signal apl, that communication was disconnected
                wx.PostEvent(self._notify_window, StatusEvent([666]))
                wx.PostEvent(
                    self._notify_window, StatusEvent([0, 1, "Error cant connect..."]))
                return None

            wx.PostEvent(self._notify_window, StatusEvent([0, 1, "Connected"]))
            wx.PostEvent(
                self._notify_window, StatusEvent([2, 1, self.port.ELMver]))
            prevstate = -1
            curstate = -1
            while self._notify_window.ThreadControl != 666:
                ...
                if curstate == 0:  # show status tab
                    ...
                    for i in range(0, len(res)):
                        wx.PostEvent(
                            self._notify_window, TestEvent([i, 1, res[i]]))
                    ...
                    for i in range(3, len(self.active)):
                        if self.active[i]:
                            ...
                            wx.PostEvent(
                                self._notify_window, ResultEvent([i, 2, "%s (%s)" % (s[1], s[2])]))
                    ...
                    if self._notify_window.ThreadControl == 1:  # clear DTC
                        ...
                        # before reset ThreadControl we must check if main
                        # thread did not want us to finish
                        if self._notify_window.ThreadControl == 666:
                            break
                    ...
                    if prevstate != 3:
                        # clear list
                        wx.PostEvent(self._notify_window, DTCEvent(0))
                        DTCCodes = self.port.get_dtc()
                        if len(DTCCodes) == 0:
                            wx.PostEvent(
                                self._notify_window, DTCEvent(["", "", "No DTC codes (codes cleared)"]))
                        for i in range(0, len(DTCCodes)):
                            wx.PostEvent(self._notify_window, DTCEvent(
                                [DTCCodes[i][1], DTCCodes[i][0], pcodes[DTCCodes[i][1]]]))
                else:
                    pass
            self.stop()
        ...
        def stop(self):
            ...
            if self.port != None:
                self.port.close()
            wx.PostEvent(
                self._notify_window, StatusEvent([0, 1, "Disconnected"]))
  ...
  # class producer end
    ...
    def sensor_control_on(self):  # after connection enable few buttons
        ...
        self.sensors.Bind(
            wx.EVT_LIST_ITEM_ACTIVATED, sensor_toggle, id=self.sensor_id)
    ...
    def build_DTC_page(self):
        ...
        self.GetDTCButton = wx.Button(
            self.DTCpanel, -1, "Get DTC", wx.Point(15, 0))
    ...
    def OnInit(self):
        ...
        self.settingmenu.Append(
            ID_RESET, "Connect", " Reopen and connect to device")
        self.settingmenu.Append(
